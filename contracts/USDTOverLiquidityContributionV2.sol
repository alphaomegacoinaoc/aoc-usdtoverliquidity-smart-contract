...
contract USDTOverLiquidityContributionV2 is
    Initializable,
    OwnableUpgradeable,
    ...
    UUPSUpgradeable
{
    ...
    event UpdatePhaseWhileRestart(
        ...
        uint256 duration,
        uint256 phaseStartTime,
        uint256 phasesEndTime
    );
    event WithdrawUSDTReferalBonus(
    ...
    event FinalizeCurrentPhase(uint256 currentPhaseIndex);
    event ReferrerUpdated(address indexed user, address oldReferrer, address newReferrer);
    ...
    constructor() {
        _disableInitializers(); // Explicitly disable initialization in the implementation contract
    }
    ...
    function initialize(address usdt) external initializer {
        require(usdt != address(0), "Invalid USDT token address");
         __Ownable_init();
    ...
    function finalizeCurrentPhase() external onlyOwner nonReentrant {
        ...
        emit FinalizeCurrentPhase(currentPhaseIndex);
    }
    ...
    /// @notice Allows users to update their referrer address
        /// @dev Updates the referrer in UserInfo and emits an event
        /// @param newReferrer The new referrer address
        function updateReferrer(address newReferrer) external nonReentrant {
            require(newReferrer != address(0), "Invalid referrer address");
            require(newReferrer != msg.sender, "Cannot refer yourself");
            address oldReferrer = userInfo[msg.sender].referrer;
            userInfo[msg.sender].referrer = newReferrer;
            emit ReferrerUpdated(msg.sender, oldReferrer, newReferrer);
        }

    /// @notice Allows users to register their wallet address and referrer in UserInfo
    /// @dev Creates or updates a UserInfo entry for the caller with the specified referrer
    /// @param referrer The address of the referrer
    function registerUser(address referrer) external nonReentrant {
    require(referrer != address(0), "Invalid referrer address");
    require(referrer != msg.sender, "Cannot refer yourself");
    require(userInfo[msg.sender].contribution == 0 &&
            userInfo[msg.sender].balanceInAOC == 0 &&
            userInfo[msg.sender].bonusInUSDT == 0 &&
            userInfo[msg.sender].bonusInAOC == 0, 
